...
(function () {
  ...
  /* ------------------------------------------------------------------
     3. TAB 2 — PETA (real Sabah SVG, 27 clickable districts)
   The geometry is inlined in dashboard.html; sabah-map.js colours it and
   builds the label overlay. The district table underneath is always present
   so the "one-tap sortable list of 27 district luas" requirement is met
   regardless of how well the map itself reads.
   ------------------------------------------------------------------ */
  function renderMap() {
    const summary = renderSabahMap({
      mountId: "sabahMap",
      period: period(),
      layer: layerSelect.value,
      selected: districtDrill,
      onSelect: function (district) {
        const row = districtSummary(period()).find(function (d) {
          return d.district === district;
        });
        if (row) openDistrictPanel(row);
      }
    });

    const max = summary ? summary.max : 0;
    const layerLabel = layerSelect.value
      ? cropName(layerSelect.value)
      : "Semua Tanaman";

    renderMapLegend({
      legendId: "mapLegend",
      note: "Lapisan: " + layerLabel +
        " · maksimum " + formatNumber(max, 1) + " ha"
    });

    renderMapTable(layerSelect.value);
  }

  /* Full sortable-by-value list of all 27 districts — guarantees the district
     figures are readable even where map labels are too small. */
  function renderMapTable(layerCropId) {
    const body = document.getElementById("mapTableBody");
    if (!body) return;

    const rows = districtSummary(period()).map(function (row) {
      return {
        district: row.district,
        luas: districtLayerLuas(row.district, period(), layerCropId),
        rekod: row.rekod,
        avgMatang: row.avgMatang,
        amaran: row.amaran
      };
    }).sort(function (a, b) { return b.luas - a.luas; });

    render(body, rows.map(function (row) {
      return el("tr", {}, [
        el("td", {}, [
          el("button", {
            type: "button", class: "link-btn", text: row.district
          })
        ]),
        ...
        el("td", { class: "num" }, [
          el("span", {
            class: matangBand(row.avgMatang),
            text: formatPct(row.avgMatang)
          })
        ]),
        el("td", { class: "num" }, [
          row.amaran > 0
    ...
    body.querySelectorAll("button").forEach(function (button) {
      button.addEventListener("click", function () {
        const row = districtSummary(period()).find(function (d) {
          return d.district === button.textContent;
        });
        if (row) openDistrictPanel(row);
      });
    });
  ...
  function renderCrops() {
    ...
    render(cropBody, sorted.flatMap(function (row) {
      const tr = el("tr", {}, [
        ...
        el("td", { class: "num" }, [trendArrowNode(row.trend)])
      ]);
