...
/* Signed delta span for KPI cards.
   The direction arrow is a CSS-drawn triangle (.d-arrow--up/down/flat), not a
   "▲"/"▼" character. Those glyphs are non-ASCII, so any encoding mismatch
   between the file and the reader renders them as mojibake — which is exactly
   what happened to this dashboard once. A CSS border triangle cannot be
   mis-decoded. The direction is also exposed in text via .sr-only so the
   meaning does not depend on the visual triangle at all. */
function deltaSpan(value, options) {
  const opts = options || {};
  const dir = value > 0 ? "up" : value < 0 ? "down" : "flat";
  const unit = opts.unit || "";
  const word = dir === "up" ? "Naik" : dir === "down" ? "Turun" : "Sama";

  const node = el("span", { class: "kpi-card__delta delta--" + dir });
  node.appendChild(el("span", { class: "d-arrow d-arrow--" + dir }));
  node.appendChild(document.createTextNode(formatDelta(value) + unit));
  node.appendChild(el("span", { class: "sr-only", text: " (" + word + ")" }));
  return node;
}

/* The same CSS triangle, for the crop table's Trend column. */
function trendArrowNode(delta) {
  const dir = delta > 0.5 ? "up" : delta < -0.5 ? "down" : "flat";
  const word = dir === "up" ? "Naik" : dir === "down" ? "Turun" : "Sama";
  const wrapper = el("span", { class: "d-arrow-wrap" });
  wrapper.appendChild(el("span", { class: "d-arrow d-arrow--" + dir }));
  wrapper.appendChild(el("span", { class: "sr-only", text: word }));
  return wrapper;
}
