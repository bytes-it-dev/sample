...
(function () {
  ...
  const editModalProvenance = document.getElementById("editModalProvenance");
  ...
  /* ------------------------------------------------------------------
     3. SECTION A — ANOMALI
     Actions: [Semak] opens the detail context, [Sunting] opens the edit editModal
  ...
  function renderAnomali() {
    ...
    render(anomaliBody, alerts.map(function (alert) {
      ...
      /* Action button labels are English at the project owner's request; the
         status vocabulary (Dihantar / Diluluskan / Ditolak) stays Malay because
         those are data values that appear in the tables and badges too. */
      const semak = el("button", {
        type: "button", class: "btn btn--ghost btn--sm", text: "Check"
      });
      semak.setAttribute("aria-label", "Check anomaly " + alert.id);
      semak.addEventListener("click", function () { inspectAlert(alert); });

      const sunting = el("button", {
        type: "button", class: "btn btn--secondary btn--sm", text: "Edit"
      });
      sunting.setAttribute("aria-label", "Edit the record behind " + alert.id);
      ...
      const tolak = el("button", {
        type: "button", class: "btn btn--danger btn--sm", text: "Reject"
      });
      tolak.setAttribute("aria-label", "Mark anomaly " + alert.id + " as resolved");
  ...
  /* [Check] — the anomaly detail, in a editModal.
     This previously wrote into a panel at the very top of the page, which sat
     above the fold while the Anomali table is far below it, so the click looked
     like it did nothing. A editModal is unmissable. */
  function inspectAlert(alert) {
    const row = findAlertRow(alert);
    openDetailModal({
      title: "Anomali " + alert.id,
      pills: [
      ...
      rows: [
        ["Tahap", SEVERITY_LABEL[alert.severity] || alert.severity],
        ["Daerah", alert.district],
        ["Tanaman", alert.cropId ? cropName(alert.cropId) : "Semua tanaman"],
        ["Isu", alert.issue],
        ["Dikesan", alert.since],
        ["Tempoh", alert.period],
        ["Ambang", "Peraturan " + alert.threshold],
        ["Rekod berkaitan", row ? row.id : "Tiada rekod sepadan"]
      ]
    });
  }

  /* Shared detail dialog for [Check] and [View]. */
  function openDetailModal(config) {
    setText("detailModalTitle", config.title);

    render(document.getElementById("detailModalPills"), config.pills || []);

    render(document.getElementById("detailModalList"), config.rows.flatMap(function (pair) {
      return [
        el("dt", { text: pair[0] }),
        el("dd", { text: pair[1] })
      ];
    ...
    document.getElementById("detailModalDoneBtn").focus();
  }

  function closeDetailModal() {
    detailModal.hidden = true;
    if (editModal.hidden) document.body.classList.remove("is-modal-open");
  }
      ...
      return el("tr", {}, [
        ...
        el("td", { class: "col-actions" }, [
          el("div", { class: "btn-group" }, [
            actionBtn("View", "btn--ghost", function () { viewRow(row); },
                      "View record " + row.id),
            actionBtn("Edit", "btn--secondary", function () { openEditModal(row); },
                      "Edit record " + row.id),
            actionBtn("Approve", "btn--success", function () { approveRow(row.id); },
                      "Approve record " + row.id),
            actionBtn("Reject", "btn--danger", function () { rejectRow(row.id); },
                      "Reject record " + row.id)
          ])
        ])
      ]);
    }));
  ...
  /* [View] — full read-only summary of the row, in the same editModal. */
  function viewRow(row) {
    openDetailModal({
      title: "Rekod " + row.id,
      pills: [statusBadge(row.status)],
      rows: [
        ["Daerah", row.district],
        ["Tanaman", cropName(row.cropId) + " (" + cropCategory(row.cropId) + ")"],
        ["Lokasi / Area", row.area],
        ["Luas", formatNumber(row.luas, 1) + " ha"],
        ["% Matang", formatPct(row.pctMatang)],
        ["Tarikh Semai", row.tarikhSemai],
        ["Dihantar Oleh", row.submittedBy || "—"],
        ["Tarikh Dihantar", row.submittedAt || "Belum dihantar"],
        ["Catatan", row.catatan || "—"],
      ...
      return el("tr", {}, [
        ...
        el("td", { class: "col-actions" }, [
          el("div", { class: "btn-group" }, [
            actionBtn("Edit", "btn--secondary", function () { openEditModal(row); },
                      "Edit record " + row.id),
            actionBtn("Cancel Approval", "btn--ghost",
                      function () { unapproveRow(row.id); },
                      "Cancel approval for record " + row.id)
          ])
        ])
      ]);
    }));
  ...
  function openEditModal(row) {
    ...
    /* Who sent it and when. The admin is editing someone else's submission, so
       the submitter and the submission date belong in front of them while they
       work — Section 4.3's edit editModal lists "all entry fields", and these are
       part of the record even though they are not editable. */
    render(editModalProvenance, [
      el("div", { class: "provenance" }, [
        el("p", { class: "provenance__row" }, [
          el("span", { class: "provenance__label", text: "Dihantar Oleh" }),
          el("span", { class: "provenance__value", text: row.submittedBy || "—" })
        ]),
        el("p", { class: "provenance__row" }, [
          el("span", { class: "provenance__label", text: "Tarikh Dihantar" }),
          el("span", {
            class: "provenance__value",
            text: row.submittedAt || "Belum dihantar"
          })
        ]),
        el("p", { class: "provenance__row" }, [
          el("span", { class: "provenance__label", text: "Jejak Audit Terakhir" }),
          el("span", { class: "provenance__value", text: auditSummary(row) })
        ])
      ])
    ]);
  ...
  document.getElementById("detailModalDoneBtn").addEventListener("click", closeDetailModal);
  ...
  /* Escape closes the editModal — expected behaviour for a dialog. */
  ...
  /* Clicking the backdrop (but not the dialog) closes the editModal. */
